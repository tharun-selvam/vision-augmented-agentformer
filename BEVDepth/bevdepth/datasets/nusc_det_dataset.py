    ...
    def get_image(self, cam_infos, cams, lidar_infos=None):
        ...
        if self.return_depth or self.use_fusion:
            ...
            for lidar_info in lidar_infos:
                lidar_path = os.path.join(self.data_root, lidar_info['LIDAR_TOP']['filename'])
                lidar_points = np.fromfile(lidar_path, dtype=np.float32, count=-1).reshape(-1, 5)[..., :4]
        ...
        for cam in cams:
            ...
            for sweep_idx, cam_info in enumerate(cam_infos):
                ...
                img_path = os.path.join(self.data_root, cam_info[cam]['filename'])
