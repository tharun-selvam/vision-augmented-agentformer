...
if __name__ == "__main__":
    ...
    # --- MODIFICATION START ---
    # Instead of iterating through splits, we will find all scenes in the user's dataset
    
    # Create output directories for all possible splits
    ...
    for split_name, scene_list in official_splits.items():
        for scene_name in scene_list:
            if split_name in ['train', 'val']: # Treat 'test' as 'val' for labels as per original script logic
                scene_to_split[scene_name] = split_name
    
    # Iterate through all scenes available in the loaded nuscenes object
    for scene in nuscenes.scene:
        ...
        if scene_name in scene_to_split:
            ...
            print(f"Skipping scene {scene_name} as it's not in train or val splits.")
            continue
            
        print(f"Processing scene: {scene_name} for split: {split}")
    ...
    # --- MODIFICATION END ---
        ...
        try:
            ...
            if not cvt_data:
                print(f"No valid data found for scene {scene_name}, skipping.")
                continue
            ...
            # --- MODIFICATION: Force garbage collection ---
            gc.collect()

        except Exception as e:
            print(f"Error processing scene {scene_name}: {e}. Skipping...")
            continue