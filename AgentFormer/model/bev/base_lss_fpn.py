...
def build_backbone(cfg):
    ...
    cfg = cfg.copy()
    backbone_type = cfg.pop('type', 'ResNet')
...
try:
    ...
    print('Import VoxelPooling fail. Using Python fallback.')

    # Fallback Python implementation
    def voxel_pooling_inference(geom_xyz, depth, features, voxel_num):
        """Simple scatter-based voxel pooling fallback (for inference only)"""
        import torch
        # geom_xyz: [batch, num_cams, D, H_geom, W_geom, 3]
        # depth: [batch*num_cams, D, H_depth, W_depth]  (already flattened!)
        # features: [batch*num_cams, channels, H_feat, W_feat]  (already flattened!)
        batch_size = geom_xyz.shape[0]
        num_cams = geom_xyz.shape[1]
        D = geom_xyz.shape[2]
        geom_H, geom_W = geom_xyz.shape[3], geom_xyz.shape[4]

        channels = features.shape[1]
        depth_H, depth_W = depth.shape[2], depth.shape[3]
        feat_H, feat_W = features.shape[2], features.shape[3]

        # Number of points from depth/features (they have same spatial dims)
        num_points = D * depth_H * depth_W

        # Create output tensor [batch, channels, Z, X, Y]
        output = torch.zeros(batch_size, channels, voxel_num[2].item(), voxel_num[0].item(), voxel_num[1].item(),
                           device=geom_xyz.device, dtype=features.dtype)

        # Upsample geometry to match depth/feature resolution using interpolation
        # geom_xyz: [batch, num_cams, D, H_geom, W_geom, 3] -> [batch, num_cams, D, H_depth, W_depth, 3]
        import torch.nn.functional as F
        if geom_H != depth_H or geom_W != depth_W:
            # Convert to float for interpolation
            geom_xyz_float = geom_xyz.float()
            # Reshape for interpolation: [B, N, D, H, W, 3] -> [B*N*3, D, H, W]
            geom_for_interp = geom_xyz_float.permute(0, 1, 5, 2, 3, 4).reshape(batch_size * num_cams * 3, D, geom_H, geom_W)
            # Interpolate to match depth resolution
            geom_upsampled = F.interpolate(geom_for_interp, size=(depth_H, depth_W), mode='bilinear', align_corners=False)
            # Reshape back: [B*N*3, D, H_depth, W_depth] -> [B, N, D, H_depth, W_depth, 3]
            geom_xyz = geom_upsampled.reshape(batch_size, num_cams, 3, D, depth_H, depth_W).permute(0, 1, 3, 4, 5, 2)
            # Convert back to int
            geom_xyz = geom_xyz.int()

        # Weight features by depth (similar to training path)
        # depth: [B*N, D, H, W], features: [B*N, C, H, W]
        # Expand and multiply: [B*N, C, D, H, W]
        img_feat_with_depth = depth.unsqueeze(1) * features.unsqueeze(2)

        # Flatten everything for processing
        # geom_xyz: [batch, num_cams, D, H, W, 3] -> [batch*num_cams, D*H*W, 3]
        geom_flat = geom_xyz.reshape(batch_size * num_cams, num_points, 3)
        # img_feat_with_depth: [B*N, C, D, H, W] -> [B*N, C, D*H*W]
        features_flat = img_feat_with_depth.reshape(batch_size * num_cams, channels, num_points)

        # Process each batch
        for b in range(batch_size):
            for c in range(num_cams):
                idx = b * num_cams + c
                coords = geom_flat[idx]  # [num_points, 3]
                feat = features_flat[idx]  # [channels, num_points] - already weighted by depth

                # Filter valid coordinates
                mask = (coords[:, 0] >= 0) & (coords[:, 0] < voxel_num[0]) & \
                       (coords[:, 1] >= 0) & (coords[:, 1] < voxel_num[1]) & \
                       (coords[:, 2] >= 0) & (coords[:, 2] < voxel_num[2])

                if mask.sum() == 0:
                    continue

                valid_coords = coords[mask].long()
                valid_feat = feat[:, mask]  # Already weighted by depth

                # Scatter into voxel grid (simple averaging)
                for i in range(valid_coords.shape[0]):
                    x, y, z = valid_coords[i]
                    output[b, :, z, x, y] += valid_feat[:, i]

        return output

    # For training, just use the inference version (since we're freezing anyway)
    voxel_pooling_train = voxel_pooling_inference
