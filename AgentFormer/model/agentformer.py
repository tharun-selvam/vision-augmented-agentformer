...
def world_to_bev_coords(agent_pos, bev_shape, x_bound, y_bound):
    """
    Convert world coordinates to normalized BEV grid coordinates for grid_sample.

    Args:
        agent_pos: [N, 2] World coordinates (x, y) in meters
        bev_shape: tuple (H, W) BEV feature map spatial size
        x_bound: tuple (min, max, resolution) for x-axis
        y_bound: tuple (min, max, resolution) for y-axis

    Returns:
        coords: [N, 2] Normalized coordinates in [-1, 1] for grid_sample
    """
    # Extract bounds
    ...
    # BEV grid: x is horizontal (width), y is vertical (height)
    bev_x = (agent_pos[:, 0] - x_min) / (x_max - x_min) * bev_shape[1]  # Width
    bev_y = (agent_pos[:, 1] - y_min) / (y_max - y_min) * bev_shape[0]  # Height

    # Normalize to [-1, 1] for grid_sample
    # grid_sample expects (x, y) where x is width dimension, y is height dimension
...
class ContextEncoder(nn.Module):
    ...
    def forward(self, data):
        ...
        if 'bev_feature_map' in data and data['bev_feature_map'] is not None:
            ...
            tf_in = bev_fusion_module(tf_in_fused.squeeze(1)).unsqueeze(1)  # [T*N, 1, model_dim]
...
class AgentFormer(nn.Module):
    def __init__(self, cfg):
        ...
        self.use_bev = cfg.get('use_bev', False)
        if self.use_bev:
            from .bev.base_lss_fpn import BaseLSSFPN
            self.bev_encoder = BaseLSSFPN(**cfg.bev_encoder)

            # Freeze BEV encoder to save GPU memory (only train AgentFormer components)
            freeze_bev = cfg.get('freeze_bev_encoder', True)  # Default to frozen
            if freeze_bev:
                for param in self.bev_encoder.parameters():
                    param.requires_grad = False
                # Must call eval() AND set training=False to use voxel_pooling_inference
                self.bev_encoder.eval()
                self.bev_encoder.training = False
                print("BEV encoder frozen - only training AgentFormer + fusion components")

            # MLP expects hidden_dims as a list
            fusion_input_dim = self.ctx['tf_model_dim'] + self.bev_encoder.output_channels
            fusion_hidden = [512]  # Hidden layer size
    ...
    def set_data(self, data):
        ...
        # Handle original dataloader format: list of tensors
        ...
        self.data['fut_motion_orig'] = torch.stack(in_data['fut_motion_3D'], dim=0).to(device)
        ...
        if self.use_bev:
            if 'sweep_imgs' in in_data:
                # Hybrid dataloader: BEV data directly in data dict
                self.data['sweep_imgs'] = in_data['sweep_imgs'].to(device)
                self.data['mats_dict'] = {k: v.to(device) for k, v in in_data['mats_dict'].items()}
            elif 'bev_data' in data:
                # Legacy format: BEV data nested in bev_data dict
    ...
    def forward(self):
        ...
        # Extract BEV features if enabled
        if self.use_bev:
            # Check if pre-computed features are already loaded
            if self.data.get('bev_feature_map') is not None:
                # Use pre-computed features (already in data dict from dataloader)
                # Move to device if needed
                if not self.data['bev_feature_map'].is_cuda and self.device.type == 'cuda':
                    self.data['bev_feature_map'] = self.data['bev_feature_map'].to(self.device)
                self.data['bev_fusion_module'] = self.bev_fusion_module

            elif self.data.get('sweep_imgs') is not None:
                # Extract features on-the-fly using BEV encoder
                sweep_imgs = self.data['sweep_imgs']
                mats_dict = self.data['mats_dict']

                # Add batch dimension if needed (shape should be [B, S, C, Ch, H, W] with 6 cams)
                if len(sweep_imgs.shape) == 5:
                    # Shape is [S, C, Ch, H, W] - add batch dimension
                    sweep_imgs = sweep_imgs.unsqueeze(0)  # [1, S, C, Ch, H, W]
                    # Also add batch dimension to mats_dict tensors
                    mats_dict = {k: v.unsqueeze(0) if isinstance(v, torch.Tensor) and len(v.shape) > 0 else v
                                for k, v in mats_dict.items()}

                # Use torch.no_grad() if BEV encoder is frozen
                if self.cfg.get('freeze_bev_encoder', True):
                    # Force eval mode to use voxel_pooling_inference instead of voxel_pooling_train
                    self.bev_encoder.eval()
                    with torch.no_grad():
                        self.data['bev_feature_map'] = self.bev_encoder(sweep_imgs, mats_dict)
                else:
                    self.data['bev_feature_map'] = self.bev_encoder(sweep_imgs, mats_dict)
    ...
    def inference(self, mode='infer', sample_num=20, need_weights=False):
        if self.use_map and self.data['map_enc'] is None:
            self.data['map_enc'] = self.map_encoder(self.data['agent_maps'])
        # Extract BEV features if enabled and not already computed
        if self.use_bev and self.data.get('sweep_imgs') is not None:
            if 'bev_feature_map' not in self.data or self.data['bev_feature_map'] is None:
